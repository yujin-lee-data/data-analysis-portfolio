...
공개·교육용 자료로 운영 정합성 · 서비스 퍼널 · 커머스 구조 · 제조 품질 · 규칙 검증 5개 주제를 다룬 포트폴리오입니다.
SQL과 Python으로 현황과 규칙을 검증하고 결과를 어디까지 말할 수 있는지 정의한 뒤 무엇을 먼저 봐야 하는지 우선순위를 정했습니다.

> **모든 작업은 공개·교육용 자료 기반입니다.**
...
| [05](./05_admissions_rule_validation/) | 규칙 구조화·검증 | 규칙 테이블 4종 / 검증기 6종 | 의도적 오류 3건 전부 검출 | 결측 성적은 0점 대신 산출 중단 / 실제 입시 산출식 아님 |

> 01~04 프로젝트 폴더의 `docs/`에 분석 기획·데이터 정의·지표 정의·전처리 기준을 문서로 남겼습니다.
...
**한 것** 테이블 간 정합성 기준을 먼저 정의하고 SQL 8개로 누락·날짜 역전·금액 불일치 후보를 탐지했습니다. 결과는 '오류 확정'이 아니라 **'점검 후보'** 로 정의했습니다. 발생 건수·영향도·긴급도를 가중해 점검 우선순위를 산정했습니다.
...
![점검 우선순위](./01_operation_data_quality/figures/priority_score.png)
...
**판단** 처음에는 디바이스나 카테고리처럼 특정 환경에서 전환 문제가 크게 나타날 것이라고 봤습니다. 실제로는 환경 차이보다 **단계별 차이가 훨씬 뚜렷**했습니다. 세그먼트를 더 쪼개는 대신 전체 사용자에게 공통으로 나타나는 우선 점검 구간을 먼저 확인하는 쪽으로 분석 축을 바꿨습니다. 가설을 유지하는 것보다 **데이터에서 나타난 차이의 크기로 우선순위를 다시 정하는 것**이 중요하다고 봤습니다.
...
공개된 대학입학전형 시행계획의 수능 반영 배점표와 등급표를 pandas 규칙 테이블 4종으로 이관하고, 옮기는 코드보다 검사하는 코드를 먼저 작성해 검증기 6종을 설계했습니다.

**핵심 포인트:** 의도적으로 틀린 규칙 3건을 심어 전부 검출됨을 확인했고, 결측 성적은 0점 계산 대신 산출 중단으로 처리했습니다. 개인 연습이며 실제 입시 산출식이 아닙니다.
...
- 모든 작업은 **공개·교육용 자료 기반**입니다. 기업 내부 데이터처럼 표현하지 않습니다.
- **인과효과를 단정하지 않습니다.** 무작위 배정이 아닌 데이터에서 나온 차이는 '관찰된 반응 차이'로 씁니다.
- 결과는 **점검 후보 / 우선 점검 후보 / 반응 차이 / 추가 검증 필요** 수준으로 표현합니다.
- **데이터의 한계를 결과와 함께 적습니다.** (라벨 불균형, 변수 익명화, 선택 편향 등)
- 01~04 프로젝트는 분석 기획·데이터 정의·지표 정의·전처리 기준을 docs/에 문서로 남겨 재현 가능하게 했습니다.
...
01~04 프로젝트 폴더의 BEGINNER_GUIDE.md를 먼저 확인하세요. 공통 실행 흐름은 아래와 같습니다.
...
- **05 대학입학전형 시행계획 공개본** — 공개된 대학입학전형 시행계획을 참고해 규칙 테이블과 검증기를 개인 연습용으로 작성했습니다. 실제 입시 산출식이 아닙니다.
